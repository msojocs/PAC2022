...
int main(int argc, char **argv)
{
    ...
    delete[] aqsntemp_a;
    delete[] aqsntemp_b;
    delete[] I_eps_array_a;
    delete[] I_eps_array_b;
    delete[] wtilde_array_a;
    delete[] wtilde_array_b;
    return 0;
}
    ...
    for (int n1 = 0; n1 < number_bands; ++n1)
    {
        for (int ig = 0; ig < ncouls; ++ig)
        {
            for (int my_igp = 0; my_igp < ngpown; ++my_igp)
            {
                ...
                int pos = n1 * ncouls + igp;
                int pos3 = my_igp * ncouls + ig;
                DataType r0 = 0.5 * vcoul[igp] * wtilde_array_a[my_igp * ncouls + igp];
                DataType i0 = 0.5 * vcoul[igp] * wtilde_array_b[my_igp * ncouls + igp];
                DataType r1 = aqsntemp_a[pos] * r0 - aqsntemp_b[pos] * i0;
                DataType i1 = aqsntemp_a[pos] * i0 + aqsntemp_b[pos] * r0;
                DataType r2 = aqsmtemp_a[pos] * r1 + aqsmtemp_b[pos] * i1;
                DataType i2 = aqsmtemp_a[pos] * i1 - aqsmtemp_b[pos] * r1;
                // ComplexType sch_store1(r2, i2);

                for (int iw = nstart; iw < nend; ++iw)
                {
                    ComplexType wdiff(wx_array[iw] - wtilde_array_a[pos3],  -wtilde_array_b[pos3]);
                    DataType wdiff_a = wx_array[iw] - wtilde_array_a[pos3];
                    DataType wdiff_b = -wtilde_array_b[pos3];
                    // ComplexType delw = 
                    //     ComplexType_conj(wdiff) *
                    //     (1 / (wdiff_a * wdiff_a + wdiff_b * wdiff_b));
                    DataType delw_a = wdiff_a * (1 / (wdiff_a * wdiff_a + wdiff_b * wdiff_b));
                    DataType delw_b = -wdiff_a * (1 / (wdiff_a * wdiff_a + wdiff_b * wdiff_b));
                    // ComplexType t1(I_eps_array_a[pos3], I_eps_array_b[pos3]);
